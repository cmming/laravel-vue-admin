            ...
            <form class="form-horizontal no-margin" @submit.prevent="validateBeforeSubmit">
              ...
              <div class="form-group">
                ...
                <div :class="{'col-lg-6': true, 'has-error': (errors.has('name:required'))}">
                  <input autocomplete="off" v-validate="'required'" type="text" class="form-control input-sm" placeholder="角色名称" name="name" v-model="userRoles.details.name">
              ...
              <div class="form-group">
                ...
                <div :class="{'col-lg-6': true, 'has-error': (errors.has('desc:required'))}">
                  <input autocomplete="off" v-validate="'required'" type="text" class="form-control input-sm" placeholder="角色描述" name="desc" v-model="userRoles.details.desc">
...
export default {
  ...
  created() {
    ...
    if (route_path.indexOf('/user/roles/edit') != -1 && (this.$route.params.id)) {
      this.$store.state.userRole.userRoles.isUpdate = true;
      //查询该id的 相关资源的基本信息
      var paramObj = { vue: this, index: this.$route.params.id };
      this.$store.dispatch('USERROLEDETAIL', paramObj);
    } else if (route_path.indexOf('/user/roles/add') != -1) {
      this.$store.state.userRole.userRoles.isUpdate = false;
    } else {
      this.$router.go(-1);
    }
  },
  ...
  methods: {
    validateBeforeSubmit() {
      ...
      this.$validator.validateAll().then((result) => {
        if (result) {
          //修改
          if (this.userRoles.isUpdate) {
            var paramObj = { vue: this, index: this.$route.params.id, resData: this.userRoles.details };
            this.$store.dispatch('UPDATEUSERROLE', paramObj);

          } else {
            // 添加
            var paramObj = { vue: this, resData: this.userRoles.details };
            console.log(paramObj.resData);
            this.$store.dispatch('ADDUSERROLE', paramObj);
          }
          return;
        }
        this.$message({
