                    ...
                    <div class="smart-widget-body">
                        ...
                        <form class="form-horizontal no-margin" @submit.prevent="validateBeforeSubmit">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-lg-10">
                                    <div class="checkbox inline-block" v-for="(item,key) in userPremissions.lists">
                                        <div class="custom-checkbox">
                                            <input type="checkbox" name='permissions[]' v-model="userRoles.premissions" :id="key" class="checkbox-grey" :value="item.id">
                                            <label :for="key"></label>
                                        </div>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            //获取 当前 角色拥有的 权限
            this.$store.dispatch('USERROLEPREMISSIONS', paramObj);
            console.log(this.userRoles.premissions);
            ...
            validateBeforeSubmit() {
                ...
                this.$validator.validateAll().then((result) => {
                    if (result) {
                        //提交 角色 的权限
                        var data = {};
                        for(var i in this.userRoles.premissions){
                            data['premissions['+i+']'] = this.userRoles.premissions[i];

                        }
                        console.log(data);
                        var paramObj = { vue: this, index: this.$route.params.id, resData: data};
                        this.$store.dispatch('STOREUSERROLEPREMISSION', paramObj);
                        return;
                    }
                    this.$message({
                        type: "error",
