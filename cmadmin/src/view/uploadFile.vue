            ...
            <div class="smart-widget widget-purple">
                ...
                <div class="smart-widget-inner">
                    <div class="smart-widget-body">
                        <div class="container">
                            <div id="uploader" class="wu-example col-lg-10">
                                <!--用来存放文件信息-->
    ...
    methods: {
        ...
        refreshToken() {
            $.ajax({
                ...
                // cache: false,
                async: false,  // 同步
                url: process.env.API_ROOT + "/refreshToken",
                headers: {
                    'Authorization': localStorage.token
                },
                success: function(response, status, xhr) {
                    localStorage.token = response.data.token;
                    localStorage.expired_at = response.data.expired_at;
                    localStorage.refresh_time = response.data.refresh_time;
                    localStorage.refresh_expired_at = response.data.refresh_expired_at;
                }
            });
        },
        createUploader() {
            ...
            _this.uploader.on('uploadAccept', function(object, ret) {
                if(ret.lastmodified>localStorage.refresh_time && ret.lastmodified < localStorage.expired_at){
                    _this.refreshToken();
                }
            });
