<template>
  <el-card>
     <h3 class="box-title padding-md" style="width: 100%;">
  ...
  methods: {
    ...
    deleteSelected(data){
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          var paramObj = { vue: this,resData:data};
          this.$store.dispatch('DELETEMENUITEM',paramObj);
        }).catch(() => {
          this.$message({
            type: 'info',
            message: '已取消删除'
      ...
      this.$validator.validateAll().then((result) => {
        if (result) {
          if(this.status == 'edit'){
            ...
            if(this.menu.details.parent_id!=''){
              var dataIndex = this.menu.details.parent_id.split(',');
              addItem.id = dataIndex[0]+','+(this.menu.lists[dataIndex[0]].childMenu.length);
              this.menu.lists[dataIndex[0]].childMenu.push(addItem);
            }else{
              addItem.id = this.menu.lists.length.toString();
              this.menu.lists.push(addItem);
            }
          }
    ...
    renderContent1(h, {node, data, store}) {
      return (
        <span>
          <span>
            <span><i class={data.iconFont}></i>&nbsp;{node.label.title}</span>
          </span>
